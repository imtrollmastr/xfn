...
def load_xfn_image(xfn_path):
    pixels = []
    image_code = []

    with open(xfn_path, 'rb') as file:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test2.py <xfn_file>")
        sys.exit(1)
    ...
    try:
        pixels, width, height = load_xfn_image(xfn_path)
    except Exception as e:
        print(f"Error loading XFN image: {e}")
        sys.exit(1)


    pygame.init()
    window = pygame.display.set_mode((width, height))
    pygame.display.set_caption('XFN Image Viewer')
